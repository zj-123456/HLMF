...
from DE1 import DifferentialEvolution
...
def df_to_chinese_description(df):
    ...
    for column in column_names:
        ...
        # 拼接描述
        description += f"{column} 的摄入量范围是从 {min_value} 到 {max_value}；"
...
def parse_arguments():
    ...
    parser.add_argument("--model", type=str, default="gemma3:27b", help="Specify a specific model to use")#qwq:32b-nothink deepseek-r1:32b gemma3:27b
...
def process_file(age_range,gender,pal,date_suffix,shell):
    ...
    if date0 =="0000":
        file_path = fr"C:\code\zjcode\deepseekrecipe\result_a_recipe\results_qwq_32b_nothink_RAG_DE_sampled_output2_no_0000.csv"
    else:
        file_path = fr"C:\code\zjcode\deepseekrecipe\result_a_recipe\{str(age_range)}_{gender}_{pal}_results_qwq_32b_nothink_RAG_DE_sampled_output2_no_{date0}.csv"
    file_name = f'C:\\code\\zjcode\\deepseekrecipe\\result_a_recipe\\{str(age_range)}_{gender}_{pal}_results_qwq_32b_nothink_RAG_DE_sampled_output2_{date_suffix}.csv'
    file_name_no = f'C:\\code\\zjcode\\deepseekrecipe\\result_a_recipe\\{str(age_range)}_{gender}_{pal}_results_qwq_32b_nothink_RAG_DE_sampled_output2_no_{date_suffix}.csv'
    ...
    for index, row in data.iterrows():
        ...
        while True:
            ...
            message_content += f"{output_matched_ings2}" \
                               f"对（{orginal_recipe}），营养素的调整策略如下：{reward_orginal_recipe}。" \
                               f"请改变（{orginal_recipe}）的食材用量。输出仅包含食材名称与用量。"
            ...
            #####以下是使用优化算法提示大模型的结果
            if rate == -1:  # 表示大模型无法优化食谱，需要调用优化函数干预，优化后得到新食谱
                bounds = []

                for ingredient, amount in ingredients_orginal_recipe:
                ...
                de = DifferentialEvolution(
                    obj_func=lambda v: objective_function(v, ingredients_orginal_recipe, nutrition_df, target_nutrients),
                    bounds=bounds_array,
                    max_iter=1000
                )
                # de = DifferentialEvolution(
                #     lambda v: objective_function(v, ingredients_orginal_recipe, nutrition_df, target_nutrients),
                #     bounds_array,
                #     1000
                # )

                # best_solution_abc, best_fitness_abc, timer = abc.optimize(verbose=True)
                best_solution_DE, best_fitness_DE, timer = de.optimize(True)
                updated_ingredients_abc = [(ingredients_orginal_recipe[i][0], best_solution_DE[i]) for i in
                                           range(len(ingredients_orginal_recipe))]
                print(updated_ingredients_abc)
                nutrient_sums_abc = calculate_nutrients2(updated_ingredients_abc, nutrition_df)
                nutrient_sums2_abc = pd.DataFrame([nutrient_sums_orginal_recipe], columns=target_keys)
                suitability_abc, yes_abc, no_abc = check_suitability(nutrient_sums_abc, target_nutrients)
                yes_number_abc = count_yes_nutrients(yes_abc)
                if yes_number_abc > yes_number_orginal_recipe:
                    recipe_abc = ",".join([f"{item[0]}: {item[1]}克" for item in updated_ingredients_abc])
                    print("DE成功运行结果" + recipe_abc)
                    # f"对于食谱（{orginal_recipe}）的营养优化，{recipe_abc}的食材用量优于{recipe_deepseek}的食材用量。"
                    # 营养需求

                    # reward_recipe_abc = calculate_reward(nutrient_sums2_abc, target_nutrients2)#的{reward_recipe_abc}。"#的食材用量优于（{orginal_recipe}）。"#
                    # f"根据每100g不同食材的营养素含量：{matched_ings2}，继续改变食谱（{recipe_abc}）的食材用量，使总营养素总量接近于{description}。"
                    # f"输出仅包含食材名称与用量。"
                    message_content_abc = (
                        f"你的回答格式正确，但是内容无效。"
                        f"在营养素总量方面，（{recipe_abc}）的食材用量优于（{orginal_recipe}）。"
                        f"继续改变食谱（{recipe_abc}）的食材用量。"
                    )
                    print(message_content_abc)
                    # time.sleep(1)
                    rate_deepseek2, recipe_deepseek2 = shell.default2(message_content_abc, target_nutrients,
                                                                      yes_number_abc)  #
                    if recipe_deepseek2 == "":
                        print(rate)
                        end = 0
                        break
                    print('llm借鉴DE结果：' + recipe_deepseek2)
                    rate_list.append(rate_deepseek2)
                    if rate_deepseek2 != -1:
                        message_content = "你的回答格式正确，并且内容有效。"
                        new_recipe = recipe_deepseek2
                        print("llm借鉴DE成功")
                    else:
                        message_content = "你的回答格式正确，但是内容无效。"
                        new_recipe = recipe_abc
                        print("DE未优化llm")
                        # break
                    ...
                    print("DE结果未使食谱优化")
                    message_content = "你的回答格式正确，但是内容无效。"
                    new_recipe = orginal_recipe
                    # break
            else:  # rate按照优化的营养素具体数量给出
                message_content = "你的回答格式正确，并且内容有效。"
                print("LLM优化了食谱")
                new_recipe = recipe_deepseek

        # 保存优化结果到csv,重新计算yes, no
        if end == 0:
            message_content = "你的回答格式错误，请严格按照输出格式约束用中文回答。"
            # 保存非正常调用的序号
            results_no.append({
                'index': index,
...
def main():
    ...
    try:
        ...
        if not args.interactive:  # False
            age_ranges =[(75, 100)]#7个年龄段 #[(12, 15),(15, 18),(18, 30),(30, 50),(50, 65),(65, 75),(75, 100)]#7个年龄段
            genders = ['女']
            pals = ['低','中']#['低','中', '高']
