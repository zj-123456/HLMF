...
class ABC:
    def __init__(self, obj_func, num_variables, bounds, colony_size, num_iter, limit):
        ...
        self.colony = np.round(
            np.random.uniform(
                low=self.bounds[:, 0],
                high=self.bounds[:, 1],
                size=(self.colony_size, self.num_variables)
            ), 1
        )
        self.colony = np.clip(self.colony, self.bounds[:, 0], self.bounds[:, 1])

        self.fitness_values = np.array([self.obj_func(ind) for ind in self.colony])  # 直接计算初始适应度值
        self.trials = np.zeros(self.colony_size)  # 用于跟踪无改进次数
    ...
    def optimize(self, verbose=False):
        ...
        for i in range(self.num_iter):
            ...
            # if verbose:
            #     print(f"Iteration {i+1}, Best Fitness: {self.best_fitness}, Best Solution: {self.best_solution}")

        end = time.time()
        return self.best_solution, self.best_fitness, end - start
    ...
    def send_scout_bees(self):
        # limit_exceeds = np.where(self.trials >= self.limit)[0]
        #
        ...
        for i in limit_exceeds:
            ...
            self.colony[i] = np.clip(new_ind, self.bounds[:, 0], self.bounds[:, 1])
    ...
    def get_new_source(self, index):
        ...
        return np.clip(v_rounded, self.bounds[:, 0], self.bounds[:, 1])
...
if __name__ == '__main__':
    file_path =r'D:\code\MOBO\zj\analysis_recipes\no_add_newingredient\18_30_男_低\(18, 30)_男_低_results_gpt4o.csv'# r'D:\code\MOBO\zj\analysis_recipes\file\output_wenxin\sampled_output.csv'
    data = pd.read_csv(file_path)

    nutrition_df = load_nutrition_data()
    age_ranges =[(18, 30)]#[(7, 8)]# [(6, 7)]
    gender = ['男']  # ['男', '女']
    pal =['低'] #['低'] 中、高
    for age_range in age_ranges:
        ...
        #file_name = 'D:/code/MOBO/zj/analysis_recipes/abc/' + str(age_range) + '_女_低_sampled_output_abc.csv'
        file_name = 'D:\code\MOBO\zj\\analysis_recipes\\no_add_newingredient\\18_30_男_低/' + str(age_range) + '_男_低_results_gpt4o_abc.csv'
